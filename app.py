...
# --- 2. CSS CUSTOMIZADO (BLINDADO CONTRA ERROS DE SINTAXE) ---
ESTILO_CSS = """
<style>
    ...
    with st.spinner('Analisando extrato e calculando valores...'):
        ...
        try:
                        ...
                        for linha in texto.split('\n'):
                            for t in termos:
                                if re.search(t, linha, re.IGNORECASE):
                                    # Captura o valor financeiro com segurança
                                    ...
                                    dados.append({
                                        ...
                                        "DESCRIÇÃO": linha.strip()[:100], # Limita o tamanho do texto
                                        "VALOR (R$)": valor_final
                                    })
                                    break
            ...
            if dados:
                ...
                # Soma Segura dos Valores (Evita que o site quebre)
                ...
                for v in df["VALOR (R$)"]:
                    try:
                        ...
                        pass # Ignora erros de conversão se o banco enviar um formato estranho
            ...
            st.error("Ocorreu um erro ao processar este PDF. Verifique se o arquivo não está protegido por senha ou se é uma imagem escaneada.")
