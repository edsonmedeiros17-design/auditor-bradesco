...
# --- 2. CSS CUSTOMIZADO (DESIGN MANTIDO INTEGRALMENTE) ---
st.markdown("""
    <style>
...
# --- 7. LÓGICA DE AUDITORIA AVANÇADA (CAPTURANDO VALORES) ---
if upload and selecionados:
    with st.spinner('Analisando extrato e calculando valores...'):
                    ...
                    for linha in texto.split('\n'):
                        for t in termos:
                            if re.search(t, linha, re.IGNORECASE):
                                # Regex para capturar valores financeiros (ex: 1.250,50 ou 45,00)
                                # Ele procura números que terminam com ,XX
        ...
        if not df.empty:
            # Cálculo do Total Recuperável (Convertendo string para float para o card)
            ...
            with c2: st.markdown(f'<div class="impact-card"><p style="font-size: 0.7rem; color: #64748B;">TOTAL IDENTIFICADO</p><h2 style="color: #BFAF83;">R$ {total_float:,.2f}</h2></div>', unsafe_allow_html=True)
            ...
            st.download_button("📥 BAIXAR LAUDO TÉCNICO COMPLETO (CSV)", csv, "auditoria_medeiros_valores.csv", "text/csv")
        else:
...
st.markdown(f"""<div class="footer-signature"><p class="footer-name">Edson Medeiros</p><p class="footer-tech">CONSULTORIA & COMPLIANCE</p></div>""", unsafe_allow_html=True)
