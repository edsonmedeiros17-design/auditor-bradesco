...
RUBRICAS_MESTRE = {
    ...
    "ENCARGOS":                 r"\bENCARGOS?\b|\bENC LIMITE\b|\bLIMITE DE CRED\b|ENCARGO\s*-?\s*\d",
    "ANUIDADE":                 r"\bANUIDADE\b|CART[ÃA]O CREDITO ANUIDADE|CARTAO CREDITO ANUIDADE",
    "OPERACOES VENCIDAS":       r"OPERA[ÇC][ÕO]ES VENCIDAS",
    "DIV. EM ATRASO":           r"DIV\.?\s*EM ATRASO|D[IÍ]VIDA\s*EM\s*ATRASO",
}

# Termos de exclusão — linha não representa débito indevido
TERMOS_EXCLUSAO = r"\bTRANSF(ERENCIA)?\b|\bSALDO\b|\bSDO\b|\bSALARIO\b|\bSAL[AÁ]RIO\b|\bSAQUE\b|\bRECEB(IMENTO)?\b|\bDEPOSITO\b|\bIOF\b"

# Cabeçalho de página nova (linha que o robô deve ignorar como dado)
PADRAO_CABECALHO = r"DATA\s+HIST[ÓO]RICO|CR[EÉ]DITO\s*\(R\$\)|D[EÉ]BITO\s*\(R\$\)|SALDO\s*\(R\$\)|FOLHA\s+\d|EXTRATO\s+DE|AG[EÊ]NCIA|MOVIMENTA[ÇC][ÃA]O\s+ENTRE|NOME:"
...
def _extrair_valor(linha: str):
    # Pega o ÚLTIMO valor numérico da linha (coluna Débito é geralmente a última)
    matches = re.findall(r"(\d{1,3}(?:\.\d{3})*,\d{2})(?!\s*%)", linha)
    return matches[-1] if matches else None

def _e_exclusao(linha: str) -> bool:
    return bool(re.search(TERMOS_EXCLUSAO, linha))

def _e_cabecalho(linha: str) -> bool:
    return bool(re.search(PADRAO_CABECALHO, linha))

# --- 4. MOTOR DE AUDITORIA — DATA INFERIOR COM PERSISTÊNCIA ENTRE PÁGINAS ---
#
# REGRA CENTRAL (modelo Bradesco / data inferior):
#
#   Caso A — Rubrica COM data na mesma linha:
#     → sela imediatamente com aquela data
#
#   Caso B — Rubrica SEM data (linha isolada):
#     → entra no bloco acumulador
#     → aguarda a próxima data encontrada (mesmo que seja em outra página)
#     → quando a data aparecer, ela sela TODOS os itens do bloco
#
#   Caso C — Linha de cabeçalho / rodapé de página:
#     → ignorada completamente (não reseta o bloco)
#
#   Caso D — Linha de exclusão (SAQUE, TRANSF, IOF, etc.):
#     → itens PENDENTE de valor no bloco são descartados
#
# O bloco NÃO é resetado entre páginas — rubricas na última
# linha de uma página recebem a data da primeira linha da próxima.
#
def realizar_auditoria(arquivo, rubricas_alvo: list) -> list:
    resultados = []
    bloco: list[dict] = []   # persiste entre páginas!

    with pdfplumber.open(arquivo) as pdf:
        total_pages = len(pdf.pages)
        progress = st.progress(0, text="Iniciando leitura...")
        ...
        for n_pag, page in enumerate(pdf.pages):
            ...
            for linha in texto.split('\n'):
                ...
                # Ignora cabeçalhos/rodapés de página — não interferem no bloco
                if _e_cabecalho(linha_up):
                    continue

                data_linha  = _extrair_data(linha_up)
                valor_linha = _extrair_valor(linha_up)

                # ── Exclusão: descarta pendentes do bloco ────────────────────
                if _e_exclusao(linha_up):
                ...
                if rubrica:
                    # CASO A: rubrica + data na mesma linha → sela imediatamente
                    if data_linha and valor_linha:
                        ...
                        # CASO B: sem data → entra no bloco, aguarda data inferior
                        bloco.append({
                            "CATEGORIA": rubrica,
                ...
                # ── Valor solto → associa ao último PENDENTE do bloco ────────
                ...
                # ── DATA INFERIOR: sela todo o bloco acumulado ───────────────
                # A data encontrada é a referência de TODOS os itens acima dela
                if data_linha and bloco:
                    ...
                    for item in bloco:
                        ...
                        # itens sem valor → descartados silenciosamente
                    bloco = []

        # Fim do PDF: se ainda houver itens no bloco sem data, descartamos
        progress.empty()
...
def gerar_excel_calculos(df: pd.DataFrame, rubrica_nome: str) -> bytes:
    ...
    c.font, c.fill = font_header, fill_blue
    ...
    c.number_format = '"R$ " #,##0.00'
    c.font, c.fill = font_header, fill_peach
...
for r in RUBRICAS_MESTRE:
    if f"chk_{r}" not in st.session_state:
        st.session_state[f"chk_{r}"] = True
    if st.sidebar.checkbox(r, value=st.session_state[f"chk_{r}"], key=f"chk_{r}"):
    ...
    if not selecionadas:
        ...
        if dados:
            ...
            for col, titulo, valor in zip(
                ...
                [f"R$ {total_geral:,.2f}", f"R$ {total_geral*2:,.2f}", str(n_lancamentos), str(n_categorias)]
            ):
                with col:
                    st.markdown(
            ...
            st.markdown('<h2 style="color:#BFAF83; text-align:center;">📥 Baixar Tabelas de Cálculos</h2>', unsafe_allow_html=True)
            ...
            for i, cat in enumerate(cats):
                ...
                with cols_dl[i % 3]:
                    st.download_button(
                        label=f"📊 {cat}  |  R$ {df_cat['V_NUM'].sum():,.2f}",
                        data=excel_file,
